...
function BillingInput({ billing, handleBillingChange, editPayment }) {
    ...
    return (
        <>
            ...
            {(/^[a-z ,.'-]+$/i.test(billing.firstName) !== true &&  billing.firstName !== "") && <div className="warning">You must enter only letters as your first name</div>}

            <input value={billing.lastName || ""} name="lastName" placeholder="Last Name" onChange={handleBillingChange} required/>
            {(/^[a-z ,.'-]+$/i.test(billing.lastName) !== true && billing.lastName !== "") && <div className="warning">You must enter only letters as your last name</div>}
