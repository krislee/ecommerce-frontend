...
export default function CardForm ({ loggedIn, paymentMethod, processing, handleSubmitCardForm, handleCardChange, handleBillingChange, handleBillingStateChange, handleCardholderNameChange, cardholderName, billing, collectCVV, redisplayCardElement, closeAddNewModal, error, disabled, sameAsShipping, handleSameAsShipping, billingInputErrorDisableButton, disableButtonAfterMakingRequest, guestProcessingPayment, classes, handleMaxExpOrZipcodeLength, handleNonNumericExpirationOrZipcode }) {
    ...
    return(
        <form id={(!loggedIn() || (loggedIn() && !paymentMethod.paymentMethodID)) ? "guest-card-form" : "card-form" } onSubmit={handleSubmitCardForm}>
            ...
            {(!sameAsShipping || (!sameAsShipping && !paymentMethod.paymentMethodID)) && <BillingInput loggedIn={loggedIn} handleBillingChange={handleBillingChange} handleBillingStateChange={handleBillingStateChange} billing={billing} paymentMethod={paymentMethod} classes={classes} handleMaxExpOrZipcodeLength={handleMaxExpOrZipcodeLength} handleNonNumericExpirationOrZipcode={handleNonNumericExpirationOrZipcode}/>}
